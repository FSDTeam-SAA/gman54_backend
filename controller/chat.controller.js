...
export const getChatForUser = catchAsync(async (req, res) => {
    ...
    const chat = await Chat.find({ user: user }).populate("messages.user","name role avatar");
    sendResponse(res, {
        statusCode: httpStatus.OK,
...
export const getChatForFarm = catchAsync(async (req, res) => {
    ...
    const chat = await Chat.find({ farm: farmId }).populate("messages.user","name role avatar");
    sendResponse(res, {
        statusCode: httpStatus.OK,
        message: "Chat retrieved successfully",
        success: true,
